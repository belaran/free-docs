...
La commande \index{telnet@\texttt{telnet}}<<~{\tt telnet}~>> permet de communiquer avec une autre machine en
...
Une fois qu'une connexion a {\'e}t{\'e} ouverte, <<~{\tt telnet}~>> passe en mode
saisie. Dans ce mode, le texte tap{\'e} au clavier est envoy{\'e} {\`a} la machine
cible.

Voici quelques \index{telnet@\texttt{telnet}!commandes internes
{\`a}}commandes accessibles au niveau du prompt <<~\verb=telnet>=~>>~:
...
\index{ftp@\texttt{ftp}}<<~{\tt ftp}~>> est une famille de commandes pour les op{\'e}rations de
...
La commande \index{rlogin@\texttt{rlogin}}<<~{\tt rlogin}~>> connecte votre terminal du site local sur
...
La commande \index{rcp@\texttt{rcp}}<<~{\tt rcp}~>> est similaire {\`a} la commane <<~{\tt cp}~>>
...
Les commandes \index{rsh@\texttt{rsh}}<<~\texttt{rsh}~>> et
\index{remsh@\texttt{remsh}}<<~\texttt{remsh}~>> sont {\'e}quivalentes.
